  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const data = res.data;
        let filteredCourse = data.filter((i) => i.category == cat);
        console.log(filteredCourse, "fillll");
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      >
        <Typography
          variant="body2"
          sx={{ mb: 3, fontWeight: "600", color: "#7a6ad8" }}
        >
          All Courses
        </Typography>
        <Typography variant="h4" sx={{ fontWeight: "600" }}>
          Checkout all the available courses
        </Typography>
      </Box>
          ...
          filteredSearch?.map((item) => (
            ...
            >
              <Link to={`/course-description/${item._id}`}>
                <img
                  src={`http://localhost:5000/api/image/${item?.thumbnail}`}
                  alt=""
                  style={{
                    width: "350px",
                    height: "280px",
